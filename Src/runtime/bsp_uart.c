 ...
 * @note       this file has been modified by Alex Chi
 * @Version    V1.0.0
 * @Date       Jan-30-2018
 ***************************************(C) COPYRIGHT 2018 DJI***************************************
 */
...
void rc_callback_handler(rc_info_t *rc, uint8_t *buff) {
    ...
    rc->mx = buff[6] | (buff[7] << 8);
    rc->my = buff[8] | (buff[9] << 8);
    rc->mz = buff[10] | (buff[11] << 8);

    rc->ml = buff[12];
    rc->mr = buff[13];

    rc->kbd = buff[14] | (buff[15] << 8);

    rc->rev = buff[16] | (buff[17] << 8);
